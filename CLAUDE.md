...
│   ├── styles/         # base.css + themes/dark.css
│   └── index.js        # 진입점
...
### playerVars (YouTube IFrame API)

`origin`과 `controls=0`, `fs=0` 조합이 핵심입니다:

```
controls=0, fs=0, rel=0, modestbranding=1, iv_load_policy=3,
playsinline=1, disablekb=1, enablejsapi=1, origin=window.location.origin
```
